...
Public Class StockOut
    ...
    Private Sub data()
        Try
            Using con As New SqlConnection(connectionString)
                ...
                StockOutDataGrid.DataSource = dt
                getData()
                AutoID()
                BindStockInDataGrid()
            End Using
    ...
    Private Sub PendingRequests_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        For Each col As DataGridViewColumn In StockOutDataGrid.Columns
    ...
    Private Sub HideSensitiveColumns()
        StockOutDataGrid.Columns("ProductID").Visible = False
    End Sub

    Private Sub ProductManagenentDataGrid_CellClick(sender As Object, e As DataGridViewCellEventArgs) Handles StockOutDataGrid.CellClick
    ...
    Private Sub btnSearch_Click(sender As Object, e As EventArgs) Handles btnSearch.Click
        ...
        If Not String.IsNullOrEmpty(searchKeyword) Then
            ...
            Next
            StockOutDataGrid.DataSource = filteredTable
        Else
            StockOutDataGrid.DataSource = dt
        End If
    End Sub
        ...
        If Not String.IsNullOrEmpty(txtProductCode.Text) AndAlso IsNumeric(txtQuantity.Text) Then
            Try
                Using con As New SqlConnection(connectionString)
                    ...
                    Dim requestedQuantity As Integer = CInt(txtQuantity.Text)

                    If existingStockQuantity < requestedQuantity Then
                        MessageBox.Show("Insufficient stock.")
                        Return
                    End If

                    Dim newStockQuantity As Integer = existingStockQuantity - requestedQuantity
    ...
    Sub getData()
        Using con As New SqlConnection(connectionString)
            Try
                ...
                cmd1 = New SqlCommand("Select * from dbo.StockOut_Transactions", con)
                sd1 = New SqlDataAdapter(cmd1)
                dt1 = New DataTable()
                Dim ds1 As New DataSet
                sd1.Fill(ds1)
                StockOutDataGrid.DataSource = ds1.Tables(0)
            Finally
                If con.State = ConnectionState.Open Then
                    con.Close()
                End If
    ...
    Sub AutoID()
        Try
            Using con As New SqlConnection(connectionString)
                con.Open()
            cmd1 = New SqlCommand("SELECT TOP 1 TransactionID, TransactionNum FROM dbo.StockOut_Transactions ORDER BY TransactionID DESC", con)
            sd1 = New SqlDataAdapter(cmd1)
    ...
    Sub Transaction()
        Try
            Using con As New SqlConnection("Data Source=RONALD\SQLEXPRESS;Initial Catalog=YSSEAN;Integrated Security=True")
                ...
                Dim archiveQuery As String = "INSERT INTO dbo.StockOut_Transactions (TransactionNum, ProductCode, Category, ProductName, Quantity, Price, Date) " &
